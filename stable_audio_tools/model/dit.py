...
class DiffusionTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        patch_size=1,
        embed_dim=768, # 1536
        cond_token_dim=0, # 768
        project_cond_tokens=True, # False
        global_cond_dim=0, # 1536
        ...
        **kwargs):
        ...
        if cond_token_dim > 0:
            ...
            cond_embed_dim = cond_token_dim if not project_cond_tokens else embed_dim # False이므로 cond_token_dim. 즉, 같다!
    ...
    def _forward(
        ...
        **kwargs):
        ...
        if cross_attn_cond is not None: # 현재 세팅에서는 차원 변화 X 768
            cross_attn_cond = self.to_cond_embed(cross_attn_cond)

        if global_embed is not None: # embed_dim이 되지만 차원이 같다. 1536
    ...
    def forward(
        ...
        **kwargs):
        ...
        # CFG dropout
        if cfg_dropout_prob > 0.0: # inference시 classifier free guidance 적용을 위해서 보통 10%를 drop해서 학습한다.
